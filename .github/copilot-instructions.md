...
- **Semi-specific helpers** (parsers, validators, domain logic) → Separate module files
  - Example: `shell-story-parser.ts` with `parseShellStoriesFromAdf()` and `addCompletionMarkerToStory()` functions
  ...
  - Export functions for testing: `export function parseShellStoriesFromAdf(...)`, `export function addCompletionMarkerToStory(...)`
